...
export function fileSelector(ops: Partial<HTMLInputElement> = {}): Promise<File> {
  let resolver: PromiseResolver<File>, rejector: PromiseRejector;
...
export function entiresMap<T>(
  obj: Object,
  mapCb: (tar: [string, any], level: number, index: number) => T,
): T[] {
  function _entriesMap(tar: Object, level = 0) {
    return Object.entries(tar).map(([key, val], index) => {
      const tar = [
        key,
        Validator.isObject(val) ? _entriesMap(val, level + 1) : val,
...
export function downloadURL(url: string, rename = false, name = '默认名称') {
  if (!rename) {
    ...
    x.onload = function () {
      ...
      a.href = url;
      a.download = fileName ? fileName + '.' + suffix1 : name;
      a.click();
    };
    x.send();
  }
...
export function deCryptoAES(
  base64edValue: string,
  _key: string = Base64Util.parse('OSEjOTVoc3VwKiYkMXpxNw=='),
) {
...
export function proxyMethod<
  ...
  P extends keyof PickFunc<T>,
  R extends (...args: any) => any = T[P],
>(
  instance: T,
  method: P,
  enhanceCb: (this: T, raw: T[P], ...params: Parameters<R>) => ReturnType<R>,
): NOOP {
...
export function buildURL(
  ...
  paramRegExp = RegExp('{(.*?)}', 'gm'),
) {
  if (!url) throw Error('buildURL params url is not nullable');
  const variables = Array.from(url.matchAll(paramRegExp));
  const vars = variables.map((item) => item[1].split('.'));
  const replaceHolders = variables.map((item) => item[0]);
  let res = url;
  replaceHolders.forEach((holder, index) => {
    res = res.replace(holder, getValueCtx(context, vars[index]));
  });
  return res;
}

export function getValueCtx(context: Record<string, any>, path: string[]) {
  if (path.filter(Boolean).length === 0) return '';
  let res = context;
  path.forEach((field) => {
    res = res[field];
  });
